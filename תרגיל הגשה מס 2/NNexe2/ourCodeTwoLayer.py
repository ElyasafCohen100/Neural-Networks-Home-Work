...
# ================================= forward propagation =================================== #
def forward_propagation(X, parameters, activation_faction_name):
    # ============= Hidden Layer ========== #
    ...
    A2 = activation_function(Z2, activation_faction_name)

    # ============= Output Layer ========== #
    Z3 = parameters["W2"].dot(A2) + parameters["b3"]
    A3 = sigmoid(Z3)
...
def backward_propagation(parameters, cache, X, Y):
    ...
    dZ3 = cache["A3"] - Y
    dW3 = (1 / number_of_samples) * dZ3.dot(cache["A2"].T)
    db3 = (1 / number_of_samples) * np.sum(dZ3)

    dA2 = np.dot(parameters["W3"].T, dZ3)
    dZ2 = dA2 * relu_der(cache["Z2"])
    dW2 = (1 / number_of_samples) * np.dot(dZ2, X.T)
    db2 = (1 / number_of_samples) * np.sum(dZ2)

    dA1 = np.dot(parameters["W2"].T, dZ2)
    dZ1 = dA1 * relu_der(cache["Z1"])
    dW1 = (1 / number_of_samples) * dZ1.dot(X.T)
